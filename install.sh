...
#
# macOS, Linux, WSL, and Git Bash on Windows. For Windows PowerShell / CMD use:
...
case "$uname_s" in
  ...
  *)
  ...
  Or download manually from $RELEASES_URL"
    ;;
esac
...
case "$(uname -m)" in
  ...
  arm64|aarch64)
    if [ "$os" = "windows" ]; then
      die "no prebuilt Windows arm64 binary yet — use x64 Windows or install.ps1 on Apple Silicon Mac via Parallels"
    fi
    arch="arm64" ;;
...
if [ "$os" = "windows" ]; then
  ...
  bin_name="bettermq.exe"
  platform_label="windows-amd64"
else
...
installed_bin="$INSTALL_DIR/bin/$bin_name"
if [ "${BETTERMQ_FORCE:-}" != "1" ] \
  && [ -f "$version_file" ] \
  && [ "$(cat "$version_file" 2>/dev/null)" = "$version" ] \
  && [ -f "$installed_bin" ]; then
...
mkdir -p "$BIN_DIR"
link_name="$BIN_DIR/$BIN_NAME"
if [ "$os" = "windows" ]; then
  cp -f "$final_bin" "$BIN_DIR/bettermq.exe" 2>/dev/null || cp -f "$final_bin" "$link_name"
  link_name="$BIN_DIR/bettermq.exe"
else
  ln -sf "$final_bin" "$link_name"
fi
ok "Linked → $link_name"
...
if [ "$on_path" = "1" ]; then
  printf '  %sStart%s   %s%s serve%s\n' "$C_DIM" "$C_RESET" "$C_BOLD" "$run_cmd" "$C_RESET"
else
  printf '  %sStart%s   %s%s serve%s\n' "$C_DIM" "$C_RESET" "$C_BOLD" "$link_name" "$C_RESET"
  printf '\n'
  if [ "$os" = "windows" ]; then
    printf '  %s(Add %s to your user PATH in Windows Settings, or use the full path above.)%s\n' "$C_DIM" "$BIN_DIR" "$C_RESET"
  else
