...
/// Load JSON from `path`, falling back to `.json.tmp`, then `.json.bak`, then `default`.
pub fn load_json_with_recovery<T, F>(path: &Path, default: F) -> JsonLoadOutcome<T>
where
    T: serde::de::DeserializeOwned,
    F: FnOnce() -> T,
{
    if !path.exists() {
        return JsonLoadOutcome {
            value: default(),
    ...
    warn!(
        file = %path.display(),
        "no valid json found; starting with empty state (delayed/cron jobs may have been lost)"
    );
    JsonLoadOutcome {
        value: default(),
        source: JsonLoadSource::Default,
...
mod tests {
    ...
    fn load_missing_file_uses_default() {
        ...
        let out = load_json_with_recovery(&path, || Sample { items: vec![] });
        assert_eq!(out.source, JsonLoadSource::Default);
    ...
    #[test]
    fn quarantines_corrupt_main_and_uses_default_when_no_fallback() {
        let dir = TempDir::new().unwrap();
        let path = sample_path(dir.path());
